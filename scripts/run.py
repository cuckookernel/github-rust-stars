...
Usage:
  ...
  scripts/run.py  <download-data|refresh-md> <lang>

  * download-data: Download data via github api for the given language
  * refresh-md: 
...
def generate_md(lang: str):
    # %%
    input_path = f"{DATA_DIR}/all-{lang}-projs.1020.json"

    with Path(input_path).open("rt") as f_in:
        print(f"Reading previously downloaded data from: {input_path}") 
    ...
    for item in items:
        ...
        if name not in names_seen:
            items_dd.append(item)
            names_seen.add(name)

    items = sorted(items_dd, key=lambda it: it['stargazers_count'], reverse=True)
    out_path = Path(f"README.md")

    with out_path.open("wt") as f_out:
        print("""_DO NOT EDIT by HAND_
